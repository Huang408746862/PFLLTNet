...
class MSEFBlock(layers.Layer):
    def __init__(self, filters, **kwargs):
        ...
        self.conv1x1 = layers.Conv2D(filters, kernel_size=(1, 1), padding='same', activation='relu')
        self.conv3x3 = layers.Conv2D(filters, kernel_size=(3, 3), padding='same', activation='relu')
        self.residual_conv = layers.Conv2D(filters, kernel_size=(1, 1), padding='same')

    def call(self, inputs):
        x = self.layer_norm(inputs)
        x1 = self.depthwise_conv(x)
        x2 = self.se_attn(x)
        x_fused = self.conv1x1(x1)
        x_fused = self.conv3x3(x_fused)
        x_residual = self.residual_conv(inputs)
        x_out = layers.Add()([x_fused, x2, x_residual])
        return x_out
...
class PixelShuffleUpsample(layers.Layer):
    ...
    def call(self, inputs):
        ...
        tf.debugging.assert_greater(new_channels, 0, message="Channels must be divisible by scale_factor^2")
        x = tf.reshape(inputs, (batch_size, height, width, self.scale_factor, self.scale_factor, new_channels))
        x = tf.transpose(x, [0, 1, 3, 2, 4, 5])
        x = tf.reshape(x, (batch_size, height * self.scale_factor, width * self.scale_factor, new_channels))
        return x

class Denoiser(Model):
    def __init__(self, num_filters, kernel_size=3, activation='relu'):
        super(Denoiser, self).__init__()
        self.conv1 = layers.Conv2D(num_filters, kernel_size=kernel_size, strides=1, padding='same',
                                   activation=activation)
        self.conv2 = layers.Conv2D(num_filters, kernel_size=kernel_size, strides=2, padding='same',
                                   activation=activation)
        self.conv3 = layers.Conv2D(num_filters, kernel_size=kernel_size, strides=2, padding='same',
                                   activation=activation)
        self.conv4 = layers.Conv2D(num_filters, kernel_size=kernel_size, strides=2, padding='same',
                                   activation=activation)
        self.bottleneck = MultiHeadSelfAttention(embed_size=num_filters, num_heads=4)
        self.up_conv2 = layers.Conv2D(num_filters * 4, kernel_size=3, padding='same', activation='relu')
        self.up2 = PixelShuffleUpsample(scale_factor=2)
        self.up_conv3 = layers.Conv2D(num_filters * 4, kernel_size=3, padding='same', activation='relu')
        self.up3 = PixelShuffleUpsample(scale_factor=2)
        self.up_conv4 = layers.Conv2D(num_filters * 4, kernel_size=3, padding='same', activation='relu')
        self.up4 = PixelShuffleUpsample(scale_factor=2)
...
class PFLLTNet(Model):
    def __init__(self, filters=32, denoiser_cb=None, denoiser_cr=None):
        ...
        self.lum_conv_adjust = layers.Conv2D(64, (1, 1), padding='same', activation='relu')
        self.lum_up = PixelShuffleUpsample(scale_factor=8)
    ...
    def call(self, inputs):
        ...
        lum_1 = self.lum_conv_adjust(lum_1)
