...
from model.arch import PFLLTNet, Denoiser
...
def train(model, dataset, loss_model, optimizer, train_dataset, test_dataset, epochs, start_epoch=1):
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        print(
            f"({get_time()}) Epoch {epoch} | GT-PSNR: {avg_gt_psnr:.2f} | PSNR: {avg_psnr:.2f} | SSIM: {avg_ssim:.3f} | loss={loss:.6f}")
...
def start_train(dataset):
    ...
    model = PFLLTNet(filters=32, denoiser_cb=denoiser_cb, denoiser_cr=denoiser_cr)
    ...
    first_decay_steps = 120 * len(train_dataset)
